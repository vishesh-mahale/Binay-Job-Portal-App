"""Unit tests for ResumeParsedRepository."""
...
@pytest.mark.asyncio
async def test_insert_parsed_result_success(mock_db):
    db, session = mock_db
...
@pytest.mark.asyncio
async def test_insert_artifact_success(mock_db):
    db, session = mock_db
    repo = ResumeParsedRepository(db)
...
@pytest.mark.asyncio
async def test_insert_event_success(mock_db):
    db, session = mock_db
    repo = ResumeParsedRepository(db)
