# Phase 09-B — Final Clean API Contract Review (Opencode)

**Auditor:** Opencode (Senior NestJS + PostgreSQL + Supabase Architect)
**Date:** 2026-08-27
**Report Path:** `04-nestjs-api/s1/phase9-b-identity-companies-api/final-contract-review/opencode-final-clean-review.md`
...
All 5 documents verified against18 verification points. Previous `session_id` BLOCKER is FIXED. Phantom columns removed. Ownership-transfer endpoint added. D1–D8 correctly reflected across all documents.

**2 remaining LOW issues:**
1. **Proposal status line** (line 3) still says "DECISIONS PENDING" — override exists at line 57 but visually stale.
2. **Proposal "Decisions required before freeze"** (lines 46–54) still lists 7 open items — all are already decided in D1–D8. Section is historical context only.
...
| 9 | `session_id` → `user_sessions.id` | ✅ PASS | Worksheet line 13 + Catalog line 11 explicitly mapped |
| 10 | All DTO fields exact SQL columns | ✅ PASS | 84 fields verified across 15 DTOs |
| 11 | `head_member_id`/`lead_member_id`/`manager_member_id` = member IDs | ✅ PASS | Catalog line 27 + Worksheet line 38 explicitly documented |
...
| 17 | No stale `NEEDS_DECISION` labels in Freeze | ✅ PASS | All resolved; override notes at lines 65-67 |
...
| D8 Ownership transfer | ✅ | ✅ | ✅ | ✅ Missing row (LOW) | ✅ `TransferOwnershipDto` | ⚠️ |
...
| 2 | `legal_name` missing from company response DTO | ✅ **FIXED** — added to Worksheet line 19 |
| 3 | MEMBERSHIP-ACCEPT/REJOIN paths not frozen | ✅ **FIXED** — paths frozen in Freeze lines 31, 34 |
| 4 | `session_id` not mapped to SQL | ✅ **FIXED** — Worksheet line 13 + Catalog line 11 |
| 5 | 7 invented DTO fields | ✅ **All 7 FIXED** |
| 6 | Stale NEEDS_DECISION labels in Freeze | ✅ **FIXED** — override notes at lines 65-67 |
| 7 | D6 rejoin-approval semantics missing | ✅ **FIXED** |
| 8 | Proposal "decisions required" outdated | ✅ **FIXED** — override note at lines 57-59 |
...
**Override note present:** "D1–D8 business decisions are resolved for the current scope." (line 65) ✅

**Membership override note present:** "Any older `NEEDS_DECISION` labels are historical placeholders." (line 67) ✅
...
| Document | Coverage |
|----------|----------|
| Proposal | "Company create must atomically initialize the existing company_settings row with approved defaults" (line 29) ✅ |
| Freeze | "company + approved settings summary" in response DTO (line 20) ✅ |
| Worksheet | "Company creation must atomically initialize the existing company_settings row with approved defaults" (line 22) ✅ |
| DTO Catalog | `CompanySummaryDto` used for response (line 12) ✅ |

`company_settings` default columns (`job_approval_required, auto_shortlist_enabled, ai_matching_enabled, notify_on_*`) are SQL-backed with defaults (04_companies.sql:328-346). Transaction must INSERT this row atomically with company creation.
...
| Document | Coverage |
|----------|----------|
| Proposal | `POST /api/v1/companies/:companyId/ownership-transfer` (line 25) ✅ |
| Freeze | `POST /api/v1/companies/:companyId/ownership-transfer` (line 36) ✅ |
| Decisions | D8: "atomic NestJS transaction" (line 72) ✅ |
| DTO Catalog | `TransferOwnershipDto` → `CompanySummaryDto` (line 22) ✅ |
...
| # | Issue | Severity | Fix |
|---|-------|----------|-----|
| L-1 | Proposal status line (line 3) says "DECISIONS PENDING" — override exists at line 57 but visually stale | LOW | Consider updating line 3 to "D1–D8 DECISIONS RESOLVED — DTO/PATH FREEZE PENDING" |
| L-2 | Proposal §"Decisions required before freeze" (lines 46–54) lists 7 open items — all decided in D1–D8 | LOW | Consider removing section or marking as "RESOLVED — see D1-D8" |
| L-3 | DTO Worksheet missing D8 transfer endpoint row | LOW | Add `TransferOwnershipDto` (`new_owner_user_id: UUID`) row |

**Zero BLOCKER or HIGH issues. Zero MEDIUM issues.**
...
**After L-1 (update status line), L-2 (clean up historical decisions section), and L-3 (add D8 to worksheet):**
```
