...
export class CreateCompanyDto {
  ...
  @Allow() @IsOptional() @IsString() legal_name?: string;
  @Allow() @IsOptional() @IsString() description?: string;
  @Allow() @IsOptional() @IsString() short_description?: string;
  @Allow() @IsOptional() @IsString() industry?: string;
  @Allow() @IsOptional() @IsString() company_size?: string;
  @Allow() @IsOptional() @IsString() website?: string;
  @Allow() @IsOptional() @IsString() linkedin_url?: string;
  @Allow() @IsOptional() @IsString() twitter_url?: string;
  @Allow() @IsOptional() @IsString() facebook_url?: string;
  @Allow() @IsOptional() @IsString() youtube_url?: string;
  @Allow() @IsOptional() @IsString() logo_path?: string;
  @Allow() @IsOptional() @IsString() cover_image_path?: string;
  @Allow() @IsOptional() @IsString() brand_color?: string;
  @Allow() @IsOptional() @IsString() address_line1?: string;
  @Allow() @IsOptional() @IsString() address_line2?: string;
  @Allow() @IsOptional() @IsString() city?: string;
  @Allow() @IsOptional() @IsString() state?: string;
  @Allow() @IsOptional() @IsString() country?: string;
  @Allow() @IsOptional() @IsString() postal_code?: string;
  @Allow() @IsOptional() @IsNumber() latitude?: number;
  @Allow() @IsOptional() @IsNumber() longitude?: number;
}
...
export class CompanyService {
  ...
  private async assertEmployer(userId: string) {
    ...
    const u = r.rows[0];
    if (!u || u.status !== 'active' || !['employer', 'admin'].includes(u.role)) {
  ...
  async getMyCompany(userId: string) {
    const r = await this.system.query(
      `SELECT ${COMPANY_RESPONSE_FIELDS} FROM public.companies c 
       WHERE c.deleted_at IS NULL AND (c.owner_id=$1 OR EXISTS (
         SELECT 1 FROM public.company_members m WHERE m.company_id=c.id AND m.user_id=$1 AND m.is_active=true
  ...
  async create(userId: string, dto: CreateCompanyDto) {
    ...
    if (!dto.name?.trim() || !dto.slug?.trim() || (!dto.email && !dto.phone)) {
      throw new BadRequestException('VALIDATION_ERROR');
    }

    return this.system.transaction(async (client) => {
      // 1. Acquire transaction-scoped advisory lock derived deterministically from owner userId
      await client.query(`SELECT pg_advisory_xact_lock(hashtext('company_create_' || $1))`, [userId]);

      // 2. Race-safe active company check on same transaction client
      const existing = await client.query(
        `SELECT id FROM public.companies WHERE owner_id = $1 AND deleted_at IS NULL`,
        [userId]
      );
      ...
      // 3. Company insert, owner member insert, and settings insert in same transaction
      const c = await client.query(
        `INSERT INTO public.companies (name,slug,legal_name,description,short_description,industry,company_size,website,linkedin_url,twitter_url,facebook_url,youtube_url,logo_path,cover_image_path,brand_color,email,phone,address_line1,address_line2,city,state,country,postal_code,latitude,longitude,owner_id) 
      ...
      await client.query(
        `INSERT INTO public.company_members (company_id,user_id,is_active,joined_at) VALUES ($1,$2,true,NOW())`,
        [company.id, userId]
      );
  ...
  async get(userId: string, companyId: string) {
    const r = await this.system.query(
      `SELECT ${COMPANY_RESPONSE_FIELDS} FROM public.companies c 
       WHERE c.id=$1 AND c.deleted_at IS NULL AND (c.owner_id=$2 OR EXISTS (
         SELECT 1 FROM public.company_members m WHERE m.company_id=c.id AND m.user_id=$2 AND m.is_active=true
       ))`,
      [companyId, userId]
    );
...
export class CompanyController {
  ...
  @Post()
  create(@Req() req: AuthReq, @Body() dto: CreateCompanyDto) {
    return this.companies.create(req.user!.sub, dto);
  }

  @Get(':companyId')
  get(@Req() req: AuthReq, @Param('companyId') id: string) {
    return this.companies.get(req.user!.sub, id);
  }

  @Patch(':companyId')
  update(@Req() req: AuthReq, @Param('companyId') id: string, @Body() dto: UpdateCompanyDto) {
    return this.companies.update(req.user!.sub, id, dto);
  }
}