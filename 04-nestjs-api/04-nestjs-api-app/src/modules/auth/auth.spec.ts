...
test('prefers the HttpOnly access-token cookie', async () => { const req = { header: () => 'Bearer header-token', cookies: { collabfor_access_token: 'cookie-token' } } as any; await expect(verifyBearer(req, 'secret', verifier)).resolves.toMatchObject({ sub: 'user-1' }); expect(verifier.verify).toHaveBeenCalledWith('cookie-token', 'secret', {}); });
